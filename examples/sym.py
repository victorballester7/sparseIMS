...
from utils.methods import Cholesky
...
def main():
    ...
    results: List[pl.DataFrame] = [gen_sym(d) for d in dimensions]
    results = [
        h.with_columns(pl.lit(d).alias('Dimension'))
        for h, d in zip(results, dimensions)
    ]
    result = pl.concat(results, how='vertical')
