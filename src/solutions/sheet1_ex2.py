...
def main():
    ...
    for i in range(len(vectors)):
        for j in range(i + 1, len(vectors)):
            dot_product = la.blas.ddot(vectors[i].flatten(), vectors[j].flatten())
    ...
    for i in range(len(vectors_orthogonal)):
        for j in range(i + 1, len(vectors_orthogonal)):
            dot_product = la.blas.ddot(vectors_orthogonal[i].flatten(), vectors_orthogonal[j].flatten())
            print(f"Dot product of {vectors_labels[i]} and {vectors_labels[j]}: {dot_product}")
